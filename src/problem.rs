use bellman::{
    gadgets::{ boolean::{ AllocatedBit, Boolean }, multipack, sha256::sha256 },
    Circuit,
    ConstraintSystem,
    SynthesisError,
};
...
impl<Scalar: PrimeField> Circuit<Scalar> for OurProblem {
    fn synthesize<CS: ConstraintSystem<Scalar>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
        ...
        let pre_bit = bit_values
            .into_iter()
            .enumerate()
            .map(|(i, b)| {
                AllocatedBit::alloc(
                    cs.namespace(|| format!("Pre bit {}", i)),
                    b
                )
            })
        ...
        let hash = impl_sha256(
            cs.namespace(|| "SHA-256(value)"),
            &pre_bit
        )?;

        multipack::pack_into_inputs(
            cs.namespace(|| "pack hash"),
            &hash
        )
    }
}
